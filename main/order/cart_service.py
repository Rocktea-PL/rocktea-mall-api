...
class CartService:
    ...
    def get_item_price(product_id, store, quantity=1):
        ...
        try:
            ...
            return Decimal(str(pricing.retail_price)) * quantity
    ...
    def update_cart_total(cart):
        ...
        for item in cart.items.all():
            item_total = CartService.get_item_price(item.product.id, cart.store, item.quantity)
    ...
    def add_or_update_item(cart, product_id, product_variant_id, quantity, store):
        ...
        if existing_item:
            ...
            existing_item.price = CartService.get_item_price(product_id, store, existing_item.quantity)
            ...
            item_price = CartService.get_item_price(product_id, store, quantity)
    ...
    def update_item_quantity(cart_item, new_quantity):
        ...
        cart_item.price = CartService.get_item_price(
